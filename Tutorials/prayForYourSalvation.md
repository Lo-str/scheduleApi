...
Instead of writing `http://localhost:3000` in every request, create a shared Axios instance in the frontend. Create a file `frontend/src/api/apiBase.ts`:
...
```ts
sessionStorage.setItem("role", role); // "employer" or "employee"
sessionStorage.setItem("name", name); // "Amara Okafor"
```
...
The login function lives in `frontend/src/api/login.ts`. It returns the result so the component can handle navigation:
...
Then in the React login component, use `navigate` after calling this function:
...
> `navigate` is a React Router hook and can only be used inside a React component — not in a plain `.ts` file. That is why the redirect logic stays in the component.
    ...
    try {
      ...
      if (role === "employee") {
        const myEntries = allEntries.filter((entry: any) =>
          entry.employees.some(
            (e: any) => e.firstName + " " + e.lastName === name,
          ),
        );
...
If the backend returns 401 (unauthorized) or 403 (forbidden), the user should be redirected to login. Add an Axios interceptor to `frontend/src/api/apiBase.ts`:
...
| Handle auth errors   | Axios interceptor in `apiBase.ts`      |
